  ...
  getProductPrice() {
    ...
    if (this.product.is_on_sale) {
      price = `<div class="s-product-card-sale-price">
                <h2>${this.getPriceFormat(this.product.sale_price)} ${pxtend}
               
                </h2>
                <span style="color:red;text-decoration-line: line-through;font-size:14px">${this.getPriceFormat(this.product?.regular_price)}
                </span>
              </div>`;
    }
        ...
        <div class="s-product-card-content" id="product-card-content-${this.product.id}">
          ...
          <div class="s-product-card-content-main ${this.isSpecial ? 's-product-card-content-extra-padding' : ''}">
            <h3 class="s-product-card-content-title nnn">
              <a href="${this.product?.url}">${(this.product?.name || '').split('-').map(part => part.replace(/ـ/g, '-')).join('<br><br>')}</a>
            </h3>
